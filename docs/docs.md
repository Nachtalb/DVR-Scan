...
This will produce output events in your working directory starting with the prefix `video.DSME_` (e.g. `video.DSME_00-01-23.456.avi`, named by the start time of each event). If you want to limit scanning to a particular part of the video frame, you can use the [region editor](#region-editor):
 ...
 * <b><pre>--thumbnails highscore</pre></b> Save a thumbnail of the frame with the highest motion score for each event. Thumbnails are named after the source video containing the event (e.g. `video.DSME_00-01-23.456.jpg`), or after the combined output file when `-o`/`--output` is used.
